...
const defaultOptions = {
    expiresIn: '1d'//1d= 1day
}

/* payload => user id, it generates auth token for every individual user  */
const createJwtToken = ({ payload, options }) => {
    const token = jwt.sign(payload.env.JWT_SECRET, {...defaultOptions, ...options});
    return token;
}

// logiv to verify token
...
const sendResWithCookies = ({ res, statusCode, user, options }) => {
    ...
    const expTime = 1000 * 60 * 60 * 24; // 24 = 24 hours
    ...
    res.status(statusCode).json({ user, token }); //Final yoken, user info after login
};
